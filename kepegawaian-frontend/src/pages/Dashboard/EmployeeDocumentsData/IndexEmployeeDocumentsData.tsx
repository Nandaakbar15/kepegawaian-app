...
import type { EmployeeDocument } from "@/src/types/EmployeeDocument";
import { Link, useNavigate } from "react-router-dom";

import Modal from "@/components/Modal";

export default function EmployeeDocumentData() {
  ...
  const fetchEmployeeDocuments = async (page: number = 1) => {
    try {
      const res = await axios.get(
        `http://localhost:3000/api/v1/employee_documents?page=${page}`,
      );
  ...
  const deleteEmployeeDocument = async (id: number) => {
    try {
      const res = await axios.delete(
        `http://localhost:3000/api/v1/deleteEmployeedocuments/${id}`,
      );
              ...
              <CardContent>
                  ...
                  <TableBody>
                    {employeeDocuments.map((data) => (
                      <TableRow key={data.id}>
                        ...
                        <TableCell className="font-medium border border-gray-300 px-4 py-2">
                          <a
                            href={`http://localhost:3000/documents/${data.file_path}`}
                        ...
                        <TableCell className="space-x-2 border border-gray-300 px-4 py-2">
                          <Link
                            to={`/edit_dokumen_pegawai/${data.id}`}
                            className="inline-block text-white rounded-lg shadow-lg px-4 py-2 bg-blue-500 hover:bg-blue-700"
                          >
                            Edit
                          </Link>
                          ...
                          <button
                            ...
                            onClick={() => deleteEmployeeDocument(data.id)}
                          >
                            Delete
                          </button>
                        </TableCell>
                ...
                <div className="flex justify-center items-center mt-6 space-x-2">
                  <button
                    ...
                    onClick={() =>
                      fetchEmployeeDocuments(paginations.current_page - 1)
                    }
                  >
                    Previous
                  </button>
