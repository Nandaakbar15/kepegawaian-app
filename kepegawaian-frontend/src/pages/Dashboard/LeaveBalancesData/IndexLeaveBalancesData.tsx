...
import type { LeaveBalances } from "@/src/types/LeaveBalances";

import Modal from "@/components/Modal";

export default function LeaveBalancesData() {
  ...
  const fetchLeaveBalances = async (page: number = 1) => {
    try {
      const res = await axios.get(
        `http://localhost:3000/api/v1/leaveBalances?page=${page}`,
      );
  ...
  const deleteLeaveBalances = async (id: number) => {
    try {
      const res = await axios.delete(
        `http://localhost:3000/api/v1/deleteBalances/${id}`,
      );
              ...
              <CardContent>
                  ...
                  <TableBody>
                    {leaveBalances.map((data) => (
                      <TableRow key={data.id}>
                        ...
                        <TableCell className="font-medium border border-gray-300 px-4 py-2">
                          {data.remaining_quota.toString()} days / months
                        </TableCell>
                        <TableCell className="space-x-2 border border-gray-300 px-4 py-2">
                          <Link
                            to={`/edit_sisa_cuti/${data.id}`}
                            className="inline-block text-white rounded-lg shadow-lg px-4 py-2 bg-blue-500 hover:bg-blue-700"
                          >
                            Edit
                          </Link>
                          ...
                          <button
                            ...
                            onClick={() => deleteLeaveBalances(data.id)}
                          >
                            Delete
                          </button>
                        </TableCell>
                ...
                <div className="flex justify-center items-center mt-6 space-x-2">
                  <button
                    ...
                    onClick={() =>
                      fetchLeaveBalances(paginations.current_page - 1)
                    }
                  >
                    Previous
                  </button>
