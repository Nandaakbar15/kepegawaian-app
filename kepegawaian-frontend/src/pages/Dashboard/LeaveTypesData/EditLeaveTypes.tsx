...
export default function EditLeavesTypeForm() {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetcLeaveTypes = async () => {
      try {
        const res = await axios.get(
          `http://localhost:3000/api/v1/leaveTypes/${id}`,
        );

        const { name, quota_days } = res.data.data;

        setName(name);
        setQuotaDays(quota_days);
      } catch (error) {
  ...
  const editLeavesTypes = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const res = await axios.put(
        `http://localhost:3000/api/v1/updateLeaveTypes/${id}`,
        {
          name: name,
      ...
      <main className="min-h-screen md:pl-62">
        ...
        <div className="mx-auto max-w-380 px-5 py-7 sm:px-8 lg:px-10 lg:py-9">
          ...
          <div className="mx-auto max-w mt-10">
            <Card>
              <CardContent>
                <form className="space-x-4 p-6" onSubmit={editLeavesTypes}>
                  <div className="mb-5">
                    <label
                      htmlFor="name"
                      className="block mb-2.5 text-sm font-medium text-heading"
                    >
                      Nama jenis cuti <span className="text-red-500">*</span>
                    </label>
                    <input
                      type="text"
                      id="name"
                      ...
                      placeholder="masukan nama jenis cuti"
                      required
                    />
                  </div>
                  <div className="mb-5">
                    <label
                      ...
                      className="block mb-2.5 text-sm font-medium text-heading"
                    >
                      Kuota Cuti <span className="text-red-500">*</span>
                    </label>
                    <input
                      type="number"
                      id="quota_days"
                      name="quota_days"
                      value={quotaDays}
                      onChange={(e) => setQuotaDays(parseInt(e.target.value))}
                      className="bg-neutral-secondary-medium border border-default-medium text-heading text-sm rounded-lg focus:ring-brand focus:border-brand block w-full px-3 py-2.5 shadow-lg placeholder:text-body"
                      placeholder="masukan kuota cuti..."
                      required
                    />
                  </div>
                  <button
