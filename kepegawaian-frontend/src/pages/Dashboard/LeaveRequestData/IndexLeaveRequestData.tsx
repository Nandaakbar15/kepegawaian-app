...
export default function LeaveRequestData() {
  ...
  const fetchLeaveRequest = async (page: number = 1) => {
    try {
      const res = await axios.get(
        `http://localhost:3000/api/v1/leaveRequest?page=${page}`,
      );
  ...
  const deleteLeaveRequest = async (id: number) => {
    try {
      const res = await axios.delete(
        `http://localhost:3000/api/v1/deleteleaveRequest/${id}`,
      );
              ...
              <CardContent>
                    ...
                    {leaveRequest.map((data) => (
                      <TableRow key={data.id}>
                        ...
                        <TableCell className="space-x-2 border border-gray-300 px-4 py-2">
                          <Link
                            to={`/edit_permintaan_cuti/${data.id}`}
                ...
                <div className="flex justify-center items-center mt-6 space-x-2">
                  <button
                    ...
                    onClick={() =>
                      fetchLeaveRequest(paginations.current_page - 1)
                    }
                  >
                    Previous
                  </button>
