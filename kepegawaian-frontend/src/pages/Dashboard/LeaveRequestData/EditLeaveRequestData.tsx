...
export default function EditLeaveRequestForm() {
  ...
  const formattedStarDate = startDate.toString().split("T")[0];
  ...
  useEffect(() => {
    const fetchLeaveRequestById = async () => {
      try {
        const res = await axios.get(
          `http://localhost:3000/api/v1/leaveRequest/${id}`,
        );
  ...
  const editLeaveRequest = async () => {
    try {
      const res = await axios.put(
        `http://localhost:3000/api/v1/updateLeaveRequest/${id}`,
        {
      ...
      <main className="min-h-screen md:pl-62">
        ...
        <div className="mx-auto max-w-380 px-5 py-7 sm:px-8 lg:px-10 lg:py-9">
          <section className="mb-8 flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
            ...
            <div>
              <h2 className="text-[29px] font-extrabold tracking-[-0.055em] text-slate-950 sm:text-[34px]">
                Form edit permintaan cuti pegawai
              </h2>
            </div>
          </section>
          <div className="mx-auto mt-10 max-w">
            <Card>
              <CardContent>
                <form className="space-x-4 p-6" onSubmit={editLeaveRequest}>
                  <div className="mb-5">
                    <label
                      htmlFor="employee_id"
